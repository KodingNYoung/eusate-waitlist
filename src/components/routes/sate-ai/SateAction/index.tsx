    ...
    <div className="flex justify-between gap-4 w-[90%]">
      <div className="w-[40%]">
        ...
        <div className="space-y-4">
          {SATE_AI_ACTIONS.map(({ key, ...action }) => (
            <ActionCard key={key} {...action} isProgress={true} />
          ))}
        </div>
      </div>
      <div className="bg-gold-100 rounded-3xl gap-4 max-w-[540px] flex justify-center flex-1"></div>
    </div>
  );
};
